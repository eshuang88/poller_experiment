...
data = pd.read_csv('/Users/y/Desktop/poller_dump/08190825_FB_food.csv')
...
word_count = dict(Counter(text_count)) # using Counter to get term frequency
...
final_data.to_csv('FB_WTF_08190825_food.csv', header=True)
...
topfive_with_duplicate_result = topfive_with_duplicate_sort.groupby('term', sort=False).apply(lambda x: x[(x.index - x.index.min()) < 3])

topfive_with_duplicate_result.to_csv('FB_5foreach_08190825_food.csv')
...
topfive_no_duplicate_sort.to_csv('FB_top20_08190825_food.csv')
'''